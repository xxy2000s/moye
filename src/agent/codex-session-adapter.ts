...
export const CODEX_SESSION_PARSER_V1 = Object.freeze({
  name: "codex-rollout",
  version: "1.0.0",
  ...
  optionsDigest: sha256(stableJson({
    ...
    duplicateMessages: "event-msg-authoritative",
    unknownRecords: "provider-system-json",
    malformedRecords: "fail-closed",
  })),
});

export interface CodexSessionAdapterOptionsV1 {
...
export interface ReadManagedCodexSessionRequestV1 {
  readonly managedArtifactRoot: string;
...
export function codexSessionSourceLocatorDigestV1(providerSessionId: string): string {
  return sha256(stableJson({ schemaVersion: 1, provider: "CODEX", kind: "CONFIRMED_THREAD_ID", providerSessionId: required(providerSessionId, "providerSessionId") }));
}

export class CodexNativeSessionAdapterV1 {
  readonly #providerSessionsRoot: string;
  readonly #managedArtifactRoot: string;
  readonly #redactor?: CodexSessionAdapterOptionsV1["redactor"];

  constructor(options: CodexSessionAdapterOptionsV1) {
  ...
  async capture(request: CaptureCodexSessionRequestV1): Promise<CapturedCodexSessionV1> {
    const intent = parseSessionTranscriptCaptureIntentV1(JSON.parse(JSON.stringify(request.intent)), request.intent.intentDigest);
    if (intent.binding.provider !== "CODEX" || intent.binding.runnerKind !== "CODEX_EXEC") {
      throw failure("UNSUPPORTED_PROVIDER", "Codex adapter requires a CODEX/CODEX_EXEC binding");
    }
    if (intent.parser.name !== CODEX_SESSION_PARSER_V1.name || intent.parser.version !== CODEX_SESSION_PARSER_V1.version ||
        intent.parser.optionsDigest !== CODEX_SESSION_PARSER_V1.optionsDigest) {
      throw failure("UNSUPPORTED_FORMAT", "Capture Intent parser identity does not match Codex rollout parser v1");
    }
    const expectedLocator = codexSessionSourceLocatorDigestV1(intent.binding.providerSessionId);
    if (intent.sourceLocatorDigest !== expectedLocator) {
      throw failure("SESSION_MISMATCH", "Capture Intent locator does not bind the confirmed Codex thread_id");
    }
    ...
    if (intent.promptBinding === "PROMPT_ENVELOPE_V1") {
      if (promptEnvelope === undefined || intent.promptEnvelope?.digest !== promptEnvelope.envelopeDigest) {
        throw failure("DIGEST_MISMATCH", "Live Codex capture requires the exact Prompt Envelope");
      }
    }
    ...
    const sourcePath = await locateRollout(providerRoot, intent.binding.providerSessionId);
...
export async function readManagedCodexSessionV1(request: ReadManagedCodexSessionRequestV1): Promise<{
  readonly manifest: SessionTranscriptManifestV1;
  readonly timeline: readonly NormalizedTimelineEventV1[];
}> {
  ...
  try {
    ...
    throw failure("MALFORMED", "Managed Codex Transcript Manifest is malformed", error);
  }
  const manifest = parseSessionTranscriptManifestV1(manifestValue, request.manifestDigest);
  if (manifest.captureId !== request.captureId || manifest.binding.provider !== "CODEX") {
    throw failure("DIGEST_MISMATCH", "Managed Codex Transcript identity does not match the requested capture");
  }
  ...
  if (sha256(normalizedBytes) !== manifest.artifacts.normalized.digest || normalizedBytes.byteLength !== manifest.artifacts.normalized.byteLength) {
    throw failure("DIGEST_MISMATCH", "Managed normalized Codex Transcript bytes do not match the Manifest");
  }
  ...
  if (manifest.artifacts.raw !== undefined) {
    ...
    if (sha256(rawBytes) !== manifest.artifacts.raw.digest || rawBytes.byteLength !== manifest.artifacts.raw.byteLength) {
      throw failure("DIGEST_MISMATCH", "Managed raw Codex Transcript bytes do not match the Manifest");
    }
  }
...
function parseCodexRollout(input: {
  ...
  try {
    ...
    throw failure("MALFORMED", "Codex rollout contains malformed JSONL", error);
  }
  if (records.length === 0) throw failure("MALFORMED", "Codex rollout is empty");
  const first = records[0]!.value;
  const firstPayload = record(first["payload"]);
  if (first["type"] !== "session_meta" || firstPayload["id"] !== input.intent.binding.providerSessionId) {
    throw failure("SESSION_MISMATCH", "Codex rollout first record does not confirm the expected thread_id");
  }
  ...
  for (const source of records) {
    ...
    if (outerType === "session_meta") {
      ...
      if (id !== undefined && id !== input.intent.binding.providerSessionId) {
        throw failure("SESSION_MISMATCH", `Codex rollout line ${source.lineNumber} changes thread_id`);
      }
      for (const candidate of [payload["parent_thread_id"], payload["forked_from_id"], record(record(record(payload["source"])["subagent"])["thread_spawn"])["parent_thread_id"]]) {
        const value = stringOrUndefined(candidate);
        if (value !== undefined) parentIds.add(value);
      }
    }
    const payloadType = stringOrUndefined(payload["type"]);
    if (outerType === "event_msg" && payloadType === "sub_agent_activity") {
      const child = stringOrUndefined(payload["agent_thread_id"]);
      if (child !== undefined) childIds.add(child);
    }
    if (outerType === "event_msg" && payloadType === "task_complete") terminalMarkerPresent = true;
    const timestamp = timestampOf(source.value, payload);
    if (timestamp.state === "PROVIDED") timestampCount += 1;
    const shape = normalizeRecord(source, outerType, payloadType, payload, renderedPromptDigest, source.lineNumber === promptRecordLine);
    ...
    timeline.push(createNormalizedTimelineEventV1({
      ...
      source: {
        ...
        partIndex: 0,
        providerType: payloadType === undefined ? outerType : `${outerType}/${payloadType}`,
        ...(stringOrUndefined(payload["id"]) === undefined ? {} : { providerId: stringOrUndefined(payload["id"])! }),
      ...
      parts: [{
        kind: shape.kind,
        content: captureContent(shape.value, input.intent.capturePolicy, input.redactor),
...
function normalizeRecord(source: ParsedRecord, outerType: string, payloadType: string | undefined, payload: Record<string, unknown>, renderedPromptDigest: string | undefined, isPromptRecord: boolean): TimelineShape {
  if (outerType === "event_msg" && payloadType === "user_message") {
    const value = required(payload["message"], `line ${source.lineNumber} user message`);
    if (isPromptRecord) return { category: "PROMPT", actor: "USER", origin: "MOYE_RENDERED_PROMPT", kind: "TEXT", value };
    return renderedPromptDigest !== undefined && sha256(value) === renderedPromptDigest ? { category: "SYSTEM", actor: "SYSTEM", origin: "PROVIDER_SYSTEM", kind: "JSON", value: stableJson(source.value) } :
      { category: "USER", actor: "USER", origin: "PROVIDER_USER", kind: "TEXT", value };
  }
  if (outerType === "response_item" && payloadType === "message" && payload["role"] === "user") {
  ...
  if (outerType === "response_item" && isToolCall(payloadType)) {
    const callId = stringOrUndefined(payload["call_id"]) ?? stringOrUndefined(payload["id"]) ?? `line-${source.lineNumber}`;
    return {
      category: "TOOL_CALL", actor: "ASSISTANT", origin: "PROVIDER_TOOL", kind: "TOOL_CALL",
      value: typeof payload["arguments"] === "string" ? payload["arguments"] : stableJson(payload["input"] ?? {}),
      toolName: stringOrUndefined(payload["name"]) ?? payloadType ?? "tool", toolCallId: callId, correlation: { toolCallId: callId },
    };
  }
  if (outerType === "response_item" && isToolResult(payloadType)) {
    const callId = stringOrUndefined(payload["call_id"]) ?? `line-${source.lineNumber}`;
    return { category: "TOOL_RESULT", actor: "TOOL", origin: "PROVIDER_TOOL", kind: "TOOL_RESULT", value: stringValue(payload["output"]), toolCallId: callId, correlation: { toolCallId: callId } };
  }
  if (outerType === "event_msg" && payloadType === "patch_apply_end") {
    const callId = stringOrUndefined(payload["call_id"]) ?? `line-${source.lineNumber}`;
    return { category: "TOOL_RESULT", actor: "TOOL", origin: "PROVIDER_TOOL", kind: "TOOL_RESULT", value: stableJson({ stdout: payload["stdout"] ?? "", stderr: payload["stderr"] ?? "", success: payload["success"] ?? null, changes: payload["changes"] ?? null }), toolCallId: callId, correlation: { toolCallId: callId } };
  }
  if (outerType === "response_item" && (payloadType === "reasoning" || payloadType === "tool_search_output")) {
    return { category: "ASSISTANT", actor: "ASSISTANT", origin: "PROVIDER_ASSISTANT", kind: "PROVIDER_EXPOSED_THINKING", value: stableJson(payload) };
  }
  const correlation = outerType === "event_msg" && payloadType === "sub_agent_activity" && stringOrUndefined(payload["agent_thread_id"]) !== undefined ?
    { childSessionId: stringOrUndefined(payload["agent_thread_id"])! } : undefined;
  return { category: "SYSTEM", actor: "SYSTEM", origin: "PROVIDER_SYSTEM", kind: "JSON", value: stableJson(source.value), ...(correlation === undefined ? {} : { correlation }) };
}

function findPromptRecordLine(records: readonly ParsedRecord[], renderedPromptDigest: string): number | undefined {
  const responseItem = records.find((source) => {
    if (source.value["type"] !== "response_item") return false;
    const payload = record(source.value["payload"]);
...
function promptTextFromResponseItem(payload: Record<string, unknown>, renderedPromptDigest: string | undefined): string | undefined {
  if (renderedPromptDigest === undefined || payload["type"] !== "message" || payload["role"] !== "user" || !Array.isArray(payload["content"])) return undefined;
  return payload["content"].find((part) => isRecord(part) && part["type"] === "input_text" && typeof part["text"] === "string" && sha256(part["text"]) === renderedPromptDigest)?.["text"] as string | undefined;
}

function parseRecords(bytes: Buffer): ParsedRecord[] {
  const text = bytes.toString("utf8");
  if (!Buffer.from(text, "utf8").equals(bytes)) throw failure("MALFORMED", "Codex rollout is not valid UTF-8");
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    try {
      ...
      throw failure("MALFORMED", `Codex rollout line ${index + 1} is malformed JSON`, error);
    }
    if (!isRecord(value)) throw failure("MALFORMED", `Codex rollout line ${index + 1} is not an object`);
...
async function locateRollout(root: string, providerSessionId: string): Promise<string> {
  const matches: string[] = [];
  const suffix = `${providerSessionId}.jsonl`;
  await walk(root);
  if (matches.length === 0) throw failure("SOURCE_MISSING", `No Codex rollout exists for confirmed thread_id ${providerSessionId}`);
  if (matches.length > 1) throw failure("SOURCE_AMBIGUOUS", `Multiple Codex rollouts exist for confirmed thread_id ${providerSessionId}`);
  return matches[0]!;
  ...
  async function walk(directory: string): Promise<void> {
    ...
    for await (const entry of stream) {
      ...
      if (entry.isSymbolicLink()) throw failure("UNSAFE_FILE_TYPE", `Symlink is forbidden below the Codex Session root: ${entry.name}`);
      if (entry.isDirectory()) await walk(candidate);
      else if (entry.isFile() && entry.name.endsWith(suffix)) matches.push(candidate);
      else if (entry.name.endsWith(suffix)) throw failure("UNSAFE_FILE_TYPE", `Codex rollout candidate is not a regular file: ${entry.name}`);
    }
  }
}
...
async function stableSnapshot(sourcePath: string, maxBytes: number): Promise<{ readonly bytes: Buffer; readonly digest: string }> {
  ...
  if (!before.isFile() || before.isSymbolicLink()) throw failure("UNSAFE_FILE_TYPE", "Codex rollout source must be a physical regular file");
  if (before.size > maxBytes) throw failure("TOO_LARGE", `Codex rollout exceeds maxSourceBytes ${maxBytes}`);
  let handle;
  try {
    handle = await open(sourcePath, fsConstants.O_RDONLY | fsConstants.O_NOFOLLOW);
    const openedBefore = await handle.stat();
    if (!openedBefore.isFile() || openedBefore.size > maxBytes) throw failure(openedBefore.size > maxBytes ? "TOO_LARGE" : "UNSAFE_FILE_TYPE", "Codex rollout failed stable source checks");
    ...
    if (openedBefore.dev !== openedAfter.dev || openedBefore.ino !== openedAfter.ino || openedBefore.size !== openedAfter.size || openedBefore.mtimeMs !== openedAfter.mtimeMs || bytes.byteLength !== openedAfter.size) {
      throw failure("SOURCE_CHANGED", "Codex rollout changed while being snapshotted");
    }
    return Object.freeze({ bytes, digest: sha256(bytes) });
  } catch (error) {
    if (error instanceof MoyeError) throw error;
    throw failure("ACCESS_DENIED", "Unable to snapshot Codex rollout safely", error);
  } finally {
    await handle?.close();
  }
}
...
function captureContent(value: string, policy: SessionCapturePolicyV1, redactor: CodexSessionAdapterOptionsV1["redactor"]): CapturedContentInputV1 {
...
function isToolCall(type: string | undefined): boolean {
  return type === "function_call" || type === "custom_tool_call" || type === "mcp_tool_call" || type === "web_search_call" || type === "tool_search_call";
}

function isToolResult(type: string | undefined): boolean {
  return type === "function_call_output" || type === "custom_tool_call_output" || type === "mcp_tool_call_output" || type === "web_search_end";
}
