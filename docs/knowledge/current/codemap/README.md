...
| `src/domain/board.ts`、`src/board/server.ts`、`public/index.html`、`public/app.js` | `/` 四列只读项目看板、Outcome/Workflow/验收历史筛选、最新成功入口、可直达/刷新的 `/tasks/<task_id>` 全屏 Task Audit Page、Definition/History/Executions 驱动的完整 SVG 状态机 Graph、Domain Event 时间线、节点 Inspector/移动 Bottom Sheet，以及全 Session 共用的 Chatbot Event Dialog | Workflow 发布精确运行元数据；遗留记录只读解析 Authority/兼容分类；路由、筛选、Events 和 Dialog 都不创建或推进状态 |
