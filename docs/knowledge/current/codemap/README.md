...
- `agent/runner.ts` 规范请求、JSONL 与 Artifact；`codex-exec.ts` 和 `claude-print.ts` 只负责 argv-only Agent 子进程，不推进 Task 状态；Claude 原生 OTel/内容采集只注入当前子进程，默认关闭；
...
| `src/agent/runner.ts`、`codex-exec.ts`、`claude-print.ts` | Agent 重复调用、JSONL 伪造、敏感内容误采集、Raw API 目录逃逸、Shell 注入 | Agent/Codex/Claude unit + Coding E2E |
