...
| `src/restate/services.ts` | TaskAuthority、TaskWorkflow、SealedTaskWorkflow、Bootstrap/Seal Recovery Workflow、ArchiveWorkflow、ProjectBoard | Authority 冻结并查询主 Workflow/append-only successor chain；Workflow 拥有 Task/Archive 流转与 durable Seal signal |
...
- `domain/core-v2-lifecycle.ts` 是 Core v2 完整 Lifecycle Reducer，覆盖 Architect/Design Review/REPLAN、Implementation Checkpoint/Repair、Documentation、两阶段 Test、Final Review、Verification Gate、Knowledge Disposition、Merge/Closure/Archive；Repair Generation 绑定上一 Candidate，旧 Attempt 不可覆盖；
...
- `TaskAuthority` 的 `SEALED_TASK_WORKFLOW` owner 让 CLI、Board Detail 和 Trace 精确查询主 Workflow或最后一个 Seal Recovery successor；每个失败 predecessor 只能追加一个 successor，source ref 保留完整失败链；
