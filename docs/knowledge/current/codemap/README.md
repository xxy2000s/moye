...
| `src/framework/project-manifest.ts`、`client.ts`、`doctor.ts`、`plugin-sdk.ts`、`builtin-adapters.ts`、`schemas/project.schema.json` | `.moye/project.yaml` v1、迁移、消费级 Task request/status/watch、Plugin API v1、七类内建 bridge、Git/target/Artifact preflight 与 doctor | Client 单次提交 owning Workflow；Plugin 只返回内容寻址 Result/Evidence，不暴露或写 Projection，不保存 Runtime token/内部 Workflow Input；默认不采集 Transcript |
...
- `scripts/session_history_acceptance.ts` 是无需点击的真实历史产品验收：只从 TaskAuthority/owning Workflow 的显式 Role Run 列表调度 enrichment，不以目录发现 Runtime 事实；保存 Task/Run/Attempt/Session、Receipt/Manifest/Source/Authority Digest、Board Event 数量、幂等重放及原 Projection 不变证明；
