...
| `src/domain/board.ts`、`src/board/server.ts`、`public/index.html`、`public/app.js` | `/` 四列只读项目看板、Event 派生的 Task 开始/结束/duration、Outcome/Workflow/验收历史筛选、最新成功入口、可直达/刷新的 `/tasks/<task_id>` 全屏 Task Audit Page、画布/角色与交付物/Workflow 状态事实/高级诊断四 Tab、完整 SVG 状态机 Graph、节点 Inspector/移动 Bottom Sheet，以及全 Session 共用的 Chatbot Event Dialog | Workflow 发布精确运行元数据；时间、路由、Tab、筛选、Events 和 Dialog 都是只读浏览状态，不创建或推进 Task |
...
- `public/app.js` 的 Core v2 审计画布默认筛选实际路径，并以紧凑主流程、节点簇范围的 Recovery/Exception 容器和独立 Archive 分区保持 Definition 完整；异常节点收敛在同一支线，背景不再横跨大半画布。最小可读缩放、未发生节点降级和直接 Task 路由优先 Trace 均只存在于浏览器内存。节点 Inspector 当前依次呈现 Agent、系统管控、状态流转、本次路径、技术 Evidence 和完整合法转换；无 Agent 节点显示 Workflow/Gate/Runner/Git/Archive 系统所有权。Domain Event 先呈现业务摘要，原始 detail 单条按需展开；
- `public/app.js` 从 Board Task 的首条 Event 派生开始时间，只在 `ARCHIVED` 后用末条 Event 显示结束时间；未结束 Task 以 Board `generatedAt` 计算运行中 duration，不新增或回写 Projection 字段。三类 Task Trace 共用四个 ARIA Tab：画布默认可见，角色/Artifact、Domain Event 与 Observer/Restate 诊断各自分层；同 Task 刷新保留选择，切换 Task 重置画布，左右键/Home/End 和窄屏滚动均为浏览器内 UI 状态；
