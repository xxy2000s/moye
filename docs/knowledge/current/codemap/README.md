...
| `src/domain/session-transcript.ts`、`src/agent/codex-session-adapter.ts`、`src/agent/claude-session-adapter.ts` | Prompt 正文与策略不一致、Provider tool result 冒充用户、Parser 版本漂移、Provider Home 越界/符号链接/读取竞态、受管 Artifact 冲突、旧 Attempt/Revision/Generation 越界、Transcript Receipt 越权或篡改 | Session Contract、Codex/Claude Adapter unit；`npm run acceptance:agent-sessions:codex` 与 `npm run acceptance:agent-sessions:claude` 真实产品证据；Workflow 恢复与历史导入由 M1-W04～W08 逐级补齐 |
