...
- `TaskAuthority` 让 CLI、Board Detail 和 Trace 精确查询主 Workflow 或合法 Recovery successor；Sealed Task 支持 append-only 修正链，Core v2 历史失败只允许一个 `CoreV2FailureRecoveryWorkflow` successor，source ref 与 source digest 保留原失败链；
