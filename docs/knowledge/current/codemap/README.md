...
| `public/index.html`、`public/app.js` | 四列只读项目看板、居中且画布优先的 Task Audit Workspace、Definition/History/Executions 驱动的完整 SVG 状态机 Graph、Agent Activity 优先且内联真实 Events 预览的桌面 Inspector/移动 Bottom Sheet，以及全 Session 共用的 Chatbot Event Dialog | 只读 Projection；Events 预览、视觉布局、筛选、折叠和 Dialog 都不创建或推进状态 |
