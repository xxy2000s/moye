...
| `src/framework/project-manifest.ts`、`client.ts`、`doctor.ts`、`schemas/project.schema.json` | `.moye/project.yaml` v1、迁移、消费级 Task request/status/watch、Git/target/Artifact preflight 与 doctor | Client 单次提交 owning Workflow；不暴露或写 Projection，不保存 Runtime token/内部 Workflow Input；默认不采集 Transcript |
