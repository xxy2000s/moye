...
| `src/domain/board.ts`、`src/board/server.ts`、`public/index.html`、`public/app.js` | `/` 四列只读项目看板、Event 派生的 Task 开始/结束/duration、Outcome/Workflow/验收历史筛选、最新成功入口、可直达/刷新的 `/tasks/<task_id>` 全屏 Task Audit Page、画布/角色与交付物/Workflow 状态事实/高级诊断四 Tab、完整 SVG 状态机 Graph、节点 Inspector/移动 Bottom Sheet、统一 Execution Ledger，以及全 Session 共用的 Chatbot Event Dialog | Workflow 发布精确运行元数据；时间、路由、Tab、筛选、Ledger、Events 和 Dialog 都是只读浏览状态，不创建或推进 Task |
...
| `src/domain/session-transcript.ts`、`src/agent/codex-session-adapter.ts` | Prompt 正文与策略不一致、Provider tool result 冒充用户、Parser 版本漂移、Provider Home 越界/符号链接/读取竞态、受管 Artifact 冲突、旧 Attempt/Revision/Generation 越界、Transcript Receipt 越权或篡改 | `tests/unit/session-transcript.test.ts`、`tests/unit/codex-session-adapter.test.ts`、`npm run acceptance:agent-sessions:codex` 真实 Codex 产品证据；Claude、Workflow 恢复与历史导入由 M1-W03～W08 逐级补齐 |
