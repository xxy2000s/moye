...
- `domain/core-v2-lifecycle.ts` 是 Core v2 Lifecycle Reducer，覆盖 Architect/Design Review/REPLAN、Implementation Checkpoint/Repair、Documentation、两阶段 Test、Final Review、Verification Gate、Knowledge Disposition，以及成功/失败 Closure 与 Archive Pending/Failed/Archived；Repair Generation 绑定上一 Candidate，旧 Attempt 不可覆盖；
- `archive/core-v2-failure.ts` 在 `<artifactRoot>/<taskId>/...` 下以 content-checked pending/rename 持久化 Failure Artifact、Failure Closure 与 Archive Receipt；共享 Artifact Root 中的不同 Task 互不覆盖，相同 Task/Closure 只接受相同内容；
- `testing/trusted-test-runner.ts` 是 argv-only 真实测试执行 Effect Adapter，持久化 Intent、逐 Case 退出码/stdout/stderr 与 Manifest；文件 Digest 绑定原始 Evidence 字节，Manifest 复用会重新校验文件，在 Intent-only 恢复时返回 UNKNOWN；
- `domain/core-v2-observer.ts` 是 Core v2 确定性只读投影，汇总 Lifecycle/Attempt 的 trace、失败、恢复和 UNKNOWN 事实；
- `restate/core-v2-services.ts` 是 keyed `CoreV2Workflow` 产品编排：只由 Workflow 推进 Task，调用真实 Role Runtime、受信任 Test Runner、幂等 Git Candidate checkpoint 和 Verification Gate 授权的双父 Merge Effect，并同步 ProjectBoard；Merge `update-ref` 回执丢失由同一 Step 读取 Git marker/parent/target 后返回 `ALREADY_APPLIED`；CLI 的 `core-v2-start/core-v2-status` 和 Board 的 `CORE_V2` Trace 读取同一 Projection；
...
- `TaskAuthority` 让 CLI、Board Detail 和 Trace 精确查询主 Workflow 或合法 Recovery successor；Sealed Task 的 numbered Attempt 可从第一层 recovery 或前一 Attempt 继续任意长度 append-only 修正链，Core v2 历史失败只允许一个 `CoreV2FailureRecoveryWorkflow` successor，source ref 与 source digest 保留原失败链；
