...
| `src/domain/session-transcript.ts`、`src/agent/codex-session-adapter.ts`、`src/agent/claude-session-adapter.ts`、`src/agent/session-capture-effect.ts`、`src/restate/core-v2-services.ts` | Prompt 正文与策略不一致、Provider tool result 冒充用户、Parser 版本漂移、Provider Home 越界/符号链接/读取竞态、受管 Artifact 冲突、Capture 丢回执后重跑 Agent、旧 Attempt/Revision/Generation 越界、Transcript Receipt 越权或篡改 | Session Contract、Codex/Claude/Capture Effect unit；`npm run acceptance:agent-sessions:codex`、`npm run acceptance:agent-sessions:claude` 与 `npm run acceptance:core-v2:sessions` 真实产品证据；Board API、历史导入由 M1-W05～W08 继续补齐 |
