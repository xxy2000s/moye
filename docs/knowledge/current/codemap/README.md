...
- `public/app.js` 的 Core v2 审计画布默认筛选实际路径，并以紧凑主流程/恢复异常/Archive 分区、最小可读缩放和未发生节点降级保持 Definition 完整；直接 Task 路由优先查询 Trace，再异步刷新 Board。节点 Inspector 当前依次呈现 Agent、系统管控、状态流转、本次路径、技术 Evidence 和完整合法转换；无 Agent 节点显示 Workflow/Gate/Runner/Git/Archive 系统所有权。Domain Event 先呈现业务摘要，原始 detail 单条按需展开；这些交互都只存在于浏览器内存；
