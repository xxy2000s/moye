...
- `archive/core-v2-failure.ts` 以 content-checked pending/rename 持久化 Failure Artifact、Failure Closure 与 Archive Receipt，相同 Task/Closure 只接受相同内容；
- `testing/trusted-test-runner.ts` 是 argv-only 真实测试执行 Effect Adapter，持久化 Intent、逐 Case 退出码/stdout/stderr 与 Manifest，并在 Intent-only 恢复时返回 UNKNOWN；
- `domain/core-v2-observer.ts` 是 Core v2 确定性只读投影，汇总 Lifecycle/Attempt 的 trace、失败、恢复和 UNKNOWN 事实；
- `restate/core-v2-services.ts` 是 keyed `CoreV2Workflow` 产品编排：只由 Workflow 推进 Task，调用真实 Role Runtime、受信任 Test Runner 和幂等 Git Candidate checkpoint，并同步 ProjectBoard；CLI 的 `core-v2-start/core-v2-status` 和 Board 的 `CORE_V2` Trace 读取同一 Projection；
