...
Node 进程暴露 HTTP/2 Restate Endpoint（默认 `9080`）和普通 HTTP Board（默认 `3000`）。Restate Ingress 默认为 `8080`，Admin/API 默认为 `9070`。完整分发 Compose 将 Endpoint 保持在内部网络，只把 Board/Ingress/Admin 绑定到 loopback；`register` 一次性容器在 Moye `/readyz` 确认 Restate 可达后提交 deployment。
