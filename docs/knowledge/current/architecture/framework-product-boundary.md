...
| CLI | `init`、`init --docs standard [--apply]`、`doctor`、`project validate`、`task *` | Artifact Root 选择、base SHA 冻结、Workflow dispatch 细节 |
...
`standard-docs-v1` 是显式 opt-in 的项目文档脚手架。`moye init --docs standard` 只读取物理 Git 根并输出按 portable path 排序的 `create | unchanged | conflict` 与 Digest；`--apply` 仅在整批无冲突时用 exclusive create 写入，完全相同的并发/重复执行收敛为 unchanged。普通 `init` 不批量写文档，`--force` 不能用于标准脚手架。生成结构包含项目 AGENTS、Docs/Sources/Delivery/Knowledge/Meta 索引、Backlog/Task 模板、项目内 validator、custom-policy Project Manifest 和内容寻址 Scaffold Manifest；validator 只读取 Manifest allowlist，拒绝路径/symlink/Digest 漂移，不扫描或迁移用户目录。模板升级必须使用未来显式 migration，不能重写已修改文件。
...
Documentation Policy v1 已实现 `none | conventional | moye-doc-graph | custom`。Client 把 Manifest policy 冻结到私有 Workflow Input；Core v2 在独立 Documentation Agent PASS 后执行确定性 Gate：先验证 clean worktree 与 HEAD=Candidate，再读取 `base..candidate` Git diff。`none` 记录 `NOT_REQUIRED`；`conventional` 在产品代码变化而没有项目事实文档变化时阻塞；Graph/custom 使用无 shell、受限输出和固定超时的 argv Runner。Runner 只额外注入冻结的 `MOYE_DOCUMENTATION_BASE_COMMIT/CANDIDATE_COMMIT`，让项目 validator 可按同一 Candidate diff 作确定性判定；这两个值已经由 Evidence 顶层绑定，不把环境变量当持久化状态。Evidence 绑定 Task/Revision/Generation/Base/Candidate/changed-files/command Digest，幂等写入 Artifact namespace，再成为 Final Review 依赖的 Docs Impact Payload。旧 Workflow Input 没有 policy 字段时保留 legacy command sequence，不在重放中插入新 durable step。
