...
M1 最终产品门禁 `npm run acceptance:agent-sessions` 不扫描验收目录：调用方必须用 `MOYE_AGENT_SESSION_RECOVERY_SUMMARY` 显式绑定一个 `SESSION_CAPTURE_RECOVERY` summary。Harness 先校验七种 Phase、唯一 Run/Session、COMPLETE Prompt/Manifest/Receipt 与成功 Archive，再从指定 Board 重查 Trace 和每个 `/session|timeline|events|stderr`；随后执行真实 Codex、真实 Claude 和 LIVE-006 历史 Sidecar 验收。Provider 子阶段如果因后续 Gate 失败已经完成，只能通过显式 result path 恢复，并用受管 Manifest/Digest 重新读取 Transcript，不能静默重跑或扫描临时目录。最终报告以 canonical 内容生成独立 Digest，不推进任何 Task 状态。
