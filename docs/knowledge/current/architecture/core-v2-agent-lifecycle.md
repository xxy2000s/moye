...
后续 Runtime 集成必须在 execution intent 之前持久化 Prompt Envelope，并在 Agent 启动前发布 `ActiveRoleRunLocatorV1`。领域层的 `renderRoleAgentPromptV2` 是真实 CLI Invocation 与 Prompt Evidence 共用的唯一 renderer；LIVE Prompt 必须与 `PreparedRealRoleRunV2` 的 Attempt、Run、Operation、Request、instructions Digest 和最终 rendered bytes 完全一致。Agent 完成后的 Provider 原生 Session 由独立 Capture Effect/Workflow 固化到受管 Artifact；Capture 失败或 UNKNOWN 只能恢复采集，不能重跑 Agent。Receipt 的权限恒为 `DIAGNOSTIC_SUPPLEMENT_ONLY`，不能推进 Task、满足 Verification Gate、改变 Closure/Archive 或令 stale Revision/Generation Evidence 复活。历史 Session 只允许绑定原 Role Manifest 追加 enrichment Receipt，不能补写成当时已存在的 Prompt Envelope。
...
旧 Core v2 Projection 不回填新字段。`scripts/session_history_matrix.ts` 只消费调用方显式 Task ID，逐项附着 `TranscriptEnrichmentWorkflow`，记录 `COMPLETE | PARTIAL | UNAVAILABLE`，并证明 source Projection Digest 前后不变。历史 `PARTIAL` 可以表示 Provider 内容完整但 Prompt Binding 只能标为 `UNVERIFIED`，不能提升为当时已存在的 Prompt Envelope。根据 [ADR-0009](../../decisions/adr/0009-separate-session-content-from-binding-confidence.md)，产品读取时由 `SessionEvidenceSemanticsV1` 把 Availability、Content Completeness、Binding Confidence 与 policy/provider Limitation 分开计算；legacy `captureState` 和 errors 仍原样保留，不能由 Board 或 UI 直接等同为内容缺失。2026-08-26 对 W09 六个真实 Task 的 44 个 Role Session 完成 append-only 恢复，共公开 1554 条 canonical Timeline Event；其中 `TASK-RCV-20260826114418-01-ROLE-RECOVERY` 为 7/7 Role、271 Event，原 Projection Digest 保持 `sha256:48d0fbf2…6858`。
...
截至 2026-08-24，证据严格分级：单元测试证明 Reducer、token、Digest 和 fencing 规则；确定性 Adapter E2E 证明协议组合；Restate E2E 证明 Journal、重启和幂等恢复；TASK-0048 的 16 个独立 Runtime Task 才证明真实 Codex、Git、Trusted Runner、Role Session/Event、Merge、Closure 与 Archive 的产品矩阵。统一审计重新查询 Workflow、Authority、Board、Git、Artifact 和 Document Graph，结果为 0 Finding，摘要 `sha256:96ad9fc9…de86`。LIVE-001～004 的 append-only failure recovery 和所有失败验收历史继续保留。该证据允许声明“Core v2 PoC 关键故障矩阵已验收”，不允许推导完整多 Daemon fencing、远程 PR、鉴权、多租户或生产运维已经完成。

Core v2 每次发布 Board Projection 时携带精确 `runtimeState`、`workflowKind=CORE_V2` 和输入声明的 `historyKind`；产品验收元数据只允许专用 acceptance fault-injection 环境在 Authority claim 前提交。Board Server 对旧 Projection 只读查询 TaskAuthority 补齐 Workflow kind，并以受限命名约定提供带 `LEGACY_CONVENTION` 来源的历史兼容分类，不写回 ProjectBoard。总览四列仍由业务 Closure/Archive 决定，筛选只在浏览器内改变可见卡片；最新成功入口只选择 `SUCCEEDED + ARCHIVED`。

Board 的 `CORE_V2` Trace 从 Lifecycle Event、Attempt、Session、Artifact 和确定性 Observer 重建主流程、成功/失败 Closure、successor Recovery Record 与 Archive 状态，以及 Happy Path、Repair/Replan/Reconcile/Failure/Archive 合法边。只有 History 实际经过的异常节点/边显示为本次发生；其余节点明确为“合法但本次未发生”。节点 Inspector 直接关联真实 Role Event；Event 在 Chatbot 弹窗中按对话、工具调用、工具结果、系统和错误筛选，不提供跳转下载入口。
